  ...
  cw3_world_spawner::Task3Service::Response &response)
{
  ...
  */ 
  ...
  if (most_common_shape == 1) {
    ...
    object_point.point = shape_objects[shape_index].position;
    grasp_orientation = shape_objects[shape_index].orientation;
  }
  else {
    shape_type = "cross";
  ...
  target_pose.orientation = getOrientation();
  success *= moveArm(target_pose);
  grasp_orientation = getOrientation();
  sub_flag_task3_rgb = true;
  
  ros::Duration(0.8).sleep();
  sub_flag_task3_rgb = false;

  task3Move(object_point, goal_point, shape_type, grasp_orientation); 
...
bool cw3::task3Move(geometry_msgs::PointStamped pick_point, geometry_msgs::PointStamped goal_point, std::string shape_type, geometry_msgs::Quaternion grasp_orientation)
{
  ROS_INFO("task3Move");
// 0.6; // Distance above object
  geometry_msgs::Pose view_pose;
  ...
  //success = pickObj(getPose(pick_point));
  success = Task3_pickObj(pick_point, grasp_orientation);
  if (shape_type == "cross")  goal_point.point.x += 2*task3_length;
  else{
    if (goal_point.point.y > 0) goal_point.point.y += 2*task3_height;
    if (goal_point.point.y < 0) goal_point.point.y -= 2*task3_length;
...
void cw3::Task3_imageCallback_color(const sensor_msgs::PointCloud2ConstPtr &cloud_input_msg)
{
  ...
  boost::shared_ptr<pcl::PointCloud<pcl::PointXYZRGB>> image_data_cloud_no_plane(new pcl::PointCloud<pcl::PointXYZRGB>);
  ...
  // Create a shared pointer to store the point cloud of the current cluster.
  boost::shared_ptr<pcl::PointCloud<pcl::PointXYZ>> clusters(new pcl::PointCloud<pcl::PointXYZ>);
  Task3_calOrientation(image_data_cloud_downsize, image_data_cloud_no_plane);
  if (image_data_cloud_no_plane->size() == 0) {
    ROS_ERROR("image_data_cloud_no_plane is empty. %ld", image_data_cloud_no_plane->size());
    return;
  }
...
void cw3::Task3_calOrientation(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud, const pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud_out)
{
  ...
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr plane_removed_cloud(new pcl::PointCloud<pcl::PointXYZRGB>);
  ...
  *plane_removed_cloud = *cloud_out + *line_cloud;
  ...
  pcl::PCDWriter writer;
  writer.write<pcl::PointXYZRGB>("/home/lilianwang/Desktop/image_data_cloud.pcd", *line_cloud, false);
  // writer.write<pcl::PointXYZ>("/home/mhj/Desktop/image_data_cloud_no_plane.pcd", *image_data_cloud_no_plane, false);
  ...
  for (int wpi = 0; wpi < task3_waypoints.size(); wpi++) {
    ...
    if (!isDetected(target_pose.position)) {
      ...
      if (obstacleColor == 1) {
        ...
        shape_objects.push_back(target_pose);
...
bool cw3::isDetected(geometry_msgs::Point current_object)
{
  ...
  for (int i = 0; i<shape_objects.size(); i++) {
    dist = calDistance_xy(current_object, shape_objects[i].position);
...
bool cw3::Task3_pickObj(geometry_msgs::PointStamped pick_point, geometry_msgs::Quaternion grasp_orientation)
{
  /**
  ...
  grasp_pose.position = pick_point.point;
  grasp_pose.orientation = grasp_orientation;
  
  grasp_pose.position.z = (task3_z_offset_obj + task3_height/2);
  ...
  if (shape_type == "cross") 
  {
    approach_pose.position.x = object_point.point.x + 0.5*length;
    approach_pose.position.y = object_point.point.y;
  }
  else{
    approach_pose.position.x = object_point.point.x;
    approach_pose.position.y = object_point.point.y + 0.5*length;
  }
