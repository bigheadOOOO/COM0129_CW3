  ...
  cw3_world_spawner::Task2Service::Response &response)
{
  ...
  // Recognize the shape at each position sent by request
  task2Move(ref_object_points, mystery_object_point);  
  // Response
  response.mystery_object_num = mysteryLabel;
  ROS_INFO("======== Mystery Type = %d =========", response.mystery_object_num);
  ...
  cw3_world_spawner::Task3Service::Response &response)
{
  ...
  Move to the position where all targets including boxes and baskets can be captured by the camera
     ...
     point 3: (-0.62, 0, arm_height),   orientation = 0
  ...
  */ 
  ...
  if (basket_object.y < 0) {
    ...
    search_position_3.y = search_position_3.y + 0.32;
  ...
  if (most_common_shape == 1) {
    ...
    object_length = shape_area[shape_index];
  }
  else {
    shape_type = "cross";
  ...
  ros::Duration(0.8).sleep();
  task3Move(object_point, goal_point, shape_type, object_length);
...
{
  /**
   * @brief Move to each position sent by request to recognize the object's shape
   * 
   */
  ...
  for(int i = 0; i< ref_object_points.size(); i++){
    ...
    ros::Duration(0.8).sleep(); // Sleep for 0.8 second, waiting for the arm be stable
    // Move the arm to the view pose that is above the object
    success = moveArm(view_pose);
    if (!success) ROS_WARN("move not success");
    ...
    // Store the number objects of each shape
    shapeLabel[Label]++; 
  ...
  // Append the mystery object to the shape list
  shapeLabel[Label]++;
  // The label of Mystery object
  mysteryLabel = Label;
...
bool cw3::task3Move(geometry_msgs::PointStamped pick_point, geometry_msgs::PointStamped goal_point, std::string shape_type, float object_length)
{
  ROS_INFO("task3Move");
  // Distance above object
  geometry_msgs::Pose view_pose;
  view_pose.position.z = 0.5;
  ...
  if (shape_type =="cross" && object_length < 0.13){
    //angle = 3.14159/4;
    success = Task3_pickObj(getPose(pick_point));
  }
  else{
    success = Task3_pickObj(getPose(pick_point));
  }

  if (shape_type == "cross")  goal_point.point.x += 0.3*object_length;
  else{
    if (goal_point.point.y > 0) goal_point.point.y += 0.3*object_length;
    if (goal_point.point.y < 0) goal_point.point.y -= 0.3*object_length;

  }
  success *= Task3_dropObj(getPose(goal_point));
  //success *= moveArm(view_pose);
  if (not success) {
    ROS_ERROR("Move arm to grasping pose failed");
  }
  return success;
}


bool cw3::Task3_pickObj(geometry_msgs::Pose pose)
{
  /**
  ...
  // Open the gripper  
  success *= Task3_moveGripper(gripper_open_ + object_length/5); //- 0.1*object_length
  if (not success) {
    ROS_ERROR("Open gripper prior to pick failed");
  }
  ...
  // Move the arm to the approach pose that is above the object
  success *= moveArm(approach_pose);
  if (not success) {
    ROS_ERROR("Moving arm to grasping pose failed");
  }

  // Transform the coordinates from camera frame to world frame
  transform_matrix = tfTransform(target_frame, camera_frame);
  Eigen::Quaterniond quaternion(transform_matrix.transform.rotation.w,
                                   transform_matrix.transform.rotation.x,
                                   transform_matrix.transform.rotation.y,
  ...
  // Adjust the approach & grasp pose based on the shape type and size
  // Shape = CROSS
  if (shape_type == "cross") {
    ROS_INFO("=================== Object Length, %.3f", object_length);
    ROS_INFO("=================== Pos x, %.3f, Pos y, %.3f", object_point.point.x, object_point.point.y);
    if(object_length < 0.13) {
      approach_pose.position.x = object_point.point.x + object_length/10; //+ 1.4*object_length/10;
      approach_pose.position.y = object_point.point.y;
    }
    if(object_length >= 0.13 && object_length <= 0.15) {
      approach_pose.position.x = object_point.point.x + 1.2*object_length/10; //+ 1.4*object_length/10;
      approach_pose.position.y = object_point.point.y;
    }
    if(object_length > 0.15) {
      approach_pose.position.x = object_point.point.x + 1.4*object_length/10; //+ 1.4*object_length/10;
      approach_pose.position.y = object_point.point.y;
    }
  ...
  // Shape = NOUGHT
    ...
    if(object_length > 0.15){
      ...
      approach_pose.position.y = object_point.point.y - 2.5* object_length/10;
    }
    else{
      approach_pose.position.x = object_point.point.x;// + 0.3*object_length;//- 0.7* object_length/10;// - 0.5*object_length;
      approach_pose.position.y = object_point.point.y - 2.3*object_length/10;
    }
  ...
  // success *= Task3_moveGripper(gripper_closed_ - 0.05*object_length);
  success *= Task3_moveGripper(0);
  if (not success) {
    ROS_ERROR("Closing gripper to grasp failed");
  }
  // Move back to approach pose, get ready to the destination
  success *= moveArm(approach_pose);
  if (not success) {
...
bool cw3::Task3_moveGripper(float width)
{
  /**
  ...
  */
  ...
  if (width < gripper_closed_ - 0.05*object_length)  
    width = 0;
...
bool cw3::Task3_dropObj(geometry_msgs::Pose pose)
{  
  /**
